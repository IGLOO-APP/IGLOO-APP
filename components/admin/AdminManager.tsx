...
const AdminManager: React.FC = () => {
  ...
  const [newAdminType, setNewAdminType] = useState<'support' | 'finance' | 'content' | 'super'>(
    'support'
  );
    ...
    return (
      <div className='flex flex-col items-center justify-center h-96 text-center p-8'>
        ...
        <p className='text-slate-500 max-w-md'>
          Apenas super administradores podem gerenciar outros administradores.
        </p>
      </div>
    );
  }
    ...
    <div className='p-8 space-y-8 animate-fadeIn relative'>
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-sm text-slate-500'>
            Adicione e gerencie permissões da equipe interna.
          </p>
        </div>
        <button
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {admins.map((admin) => (
          <div
            key={admin.id}
            className='bg-white dark:bg-surface-dark p-6 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm group hover:shadow-md transition-all relative overflow-hidden'
          >
            <div
              className={`absolute top-0 left-0 w-full h-1 ${
                admin.admin_type === 'super'
                  ? 'bg-purple-500'
                  : admin.admin_type === 'finance'
                    ? 'bg-emerald-500'
                    : 'bg-blue-500'
              }`}
            ></div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn p-4'>
          <div className='bg-white dark:bg-surface-dark w-full max-w-sm p-6 rounded-[32px] shadow-2xl animate-scaleIn border border-gray-100 dark:border-white/5 text-center'>
            ...
            <p className='text-sm text-slate-500 mb-6'>
              Esta ação removerá as permissões administrativas deste usuário. Ele voltará a ser um
              usuário comum (Tenant).
            </p>

    ...
    <div className='space-y-8 animate-fadeIn'>
      ...
      <div className='bg-slate-50 dark:bg-white/5 p-6 rounded-2xl border border-gray-100 dark:border-white/5'>
        ...
        <form onSubmit={createFlag} className='flex gap-4 items-end'>
          ...
          <button
            type='submit'
            className='px-6 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-xl hover:opacity-90 transition-opacity flex items-center gap-2'
          >
            <Plus size={18} />
            Criar
          </button>
      ...
      <div className='space-y-4'>
        {loading ? (
          ...
          flags.map((flag) => (
            <div
              key={flag.id}
              className='bg-white dark:bg-surface-dark p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 flex items-center justify-between group hover:border-primary/50 transition-colors'
            >
              <div className='flex items-start gap-4'>
                <div
                  className={`p-3 rounded-xl ${flag.enabled ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-50 text-slate-400 dark:bg-white/5'}`}
                >
                  <Tag size={20} />
                </div>
                <div>
                  <h4 className='font-mono font-bold text-slate-900 dark:text-white'>
                    {flag.name}
                  </h4>
              ...
              <div className='flex items-center gap-6'>
                <Switch
                  ...
                  className={`${
                    flag.enabled ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-white/10'

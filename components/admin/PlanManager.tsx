        ...
        <button
          onClick={() =>
            setEditingPlan({
              is_active: true,
              features: [],
              limits: { properties: 0, tenants: 0, storage_gb: 0, users: 0 },
            })
          }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingPlan({
                      ...editingPlan,
                      price: { ...editingPlan.price!, monthly: parseFloat(e.target.value) },
                    })
                  }
                  className='w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10'
                  required
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {loading ? (
          ...
          plans.map((plan) => (
            <div
              key={plan.id}
              className={`bg-white dark:bg-surface-dark p-6 rounded-3xl shadow-sm border ${plan.is_active ? 'border-gray-100 dark:border-white/5' : 'border-red-100 dark:border-red-500/20 opacity-75'} relative group hover:shadow-lg transition-all`}
            >
              <div className='absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity'>
                <button
                  onClick={() => setEditingPlan(plan)}
                  className='p-2 bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-white rounded-lg hover:bg-primary hover:text-white transition-colors'
                >
                  <Edit2 size={16} />
                </button>
                <button
                  onClick={() => handleDeletePlan(plan.id)}
                  className='p-2 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-colors'
                >
                  <Trash2 size={16} />
                </button>
              </div>
              ...
              <div className='space-y-3 mb-6'>
                <div className='flex justify-between text-sm'>
                  ...
                  <span className='font-bold text-slate-900 dark:text-white'>
                    {plan.limits.properties === -1 ? 'Ilimitado' : plan.limits.properties}
                  </span>
                </div>
                <div className='flex justify-between text-sm'>
              ...
              <div
                className={`text-center py-2 rounded-xl text-xs font-bold uppercase tracking-widest ${plan.is_active ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-red-50 text-red-600'}`}
              >

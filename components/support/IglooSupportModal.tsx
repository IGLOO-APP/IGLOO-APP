...
export const IglooSupportModal: React.FC<IglooSupportModalProps> = ({
  ...
  useEffect(() => {
    if (activeTicketId) {
      ...
      setTickets((prev) =>
        prev.map((t) => (t.id === activeTicketId ? ({ ...t, unreadCount: 0 } as any) : t))
      );
    } else {
      setMessages([]);
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`support_messages_user:${activeTicketId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  useEffect(() => {
    ...
    const globalChannel = supabase
      .channel(`support_global_user:${userId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fadeIn pb-safe pt-safe'>
      <div className='w-full max-w-5xl h-[85vh] bg-white dark:bg-surface-dark border border-slate-200/60 dark:border-white/5 rounded-[32px] shadow-2xl overflow-hidden flex flex-col md:flex-row animate-scaleUp premium-glass'>
        ...
        <div
          className={`w-full md:w-80 border-r border-slate-200/60 dark:border-white/5 flex flex-col bg-slate-50/50 dark:bg-black/20 shrink-0 h-full ${activeTicketId ? 'hidden md:flex' : 'flex'}`}
        >
          {/* Header */}
            ...
            {loadingTickets && tickets.length === 0 ? (
              ...
              filteredTickets.map((t) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${cat.color}`}
                    >
                      <CatIcon size={18} />
                    </div>
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='flex justify-between items-center'>
                        <span
                          className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ${getStatusBadge(t.status)}`}
                        >
                          {t.status}
                        </span>
                        {t.assignee && (
                ...
                <div className='flex items-center gap-3 min-w-0'>
                  ...
                  <div className='min-w-0'>
                    <div className='flex items-center gap-2'>
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ${getStatusBadge(activeTicket.status)}`}
                      >
                        {activeTicket.status}
                      </span>
                    </div>
                          ...
                          <div className='flex items-center gap-1.5 mt-1 px-1.5 text-[9px] text-slate-400 font-bold'>
                            ...
                            {isMe && (
                              <CheckCheck
                                size={12}
                                className={
                                  msg.is_read ? 'text-primary' : 'text-slate-300 dark:text-white/20'
                                }
                              />
                            )}
                          </div>
            ...
            <div className='flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-50/30 dark:bg-black/5'>
              ...
              <div className='max-w-sm space-y-2.5'>
                ...
                <p className='text-xs text-slate-500 leading-relaxed max-w-[280px] mx-auto'>
                  Gerencie suas dúvidas operacionais, financeiras ou técnicas de forma direta com
                  nossa equipe especializada.
                  <span className='block mt-2 font-bold text-primary'>
                    Selecione um chamado existente ou abra um novo.
                  </span>
                </p>
              </div>
            </div>

...
export const TenantOnboardingWizard: React.FC<TenantOnboardingWizardProps> = ({
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && (() => {})()}>
      <DialogContent
        className='max-h-[90vh] overflow-y-auto p-0 gap-0 md:max-w-2xl p-0 overflow-hidden'
        showCloseButton={false}
      >
        <div
          className={`min-h-[500px] flex flex-col bg-background-light dark:bg-background-dark transition-all duration-300 ${isAnimating ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0'}`}
        >
          {/* --- STEP 1: WELCOME --- */}
          {currentStep === 'welcome' && (
            <div className='flex-1 flex flex-col items-center justify-center p-8 text-center space-y-8 animate-fadeIn'>
              ...
              <div>
                ...
                <p className='text-slate-500 dark:text-slate-400 font-medium max-w-sm mx-auto'>
                  Olá, <span className='text-primary font-bold'>{tenant.name.split(' ')[0]}</span>.
                  O proprietário do imóvel{' '}
                  <span className='font-bold text-slate-700 dark:text-slate-200'>
                    {property?.name || 'seu imóvel'}
                  </span>{' '}
                  convidou você para o IGLOO.
                  <br />
                  <br />
              ...
              <button
                onClick={() =>
                  handleNext(
                    tenant.contract?.status === 'pending_signature' ? 'contract' : 'profile'
                  )
                }
            ...
            <div className='flex-1 flex flex-col animate-fadeIn'>
              <div className='p-8 pb-0'>
                <span className='px-3 py-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full text-[10px] font-black uppercase tracking-widest'>
                  Passo 01: Segurança Jurídica
                </span>
                <h2 className='text-2xl font-black text-slate-900 dark:text-white mt-4 mb-2'>
                  Assinatura Digital
                </h2>
                <p className='text-sm text-slate-500 font-medium mb-6'>
                  Revise e assine o contrato de locação para oficializar sua estadia.
                </p>
              </div>
                  ...
                  <div className='grid grid-cols-2 gap-4 py-4 border-y border-slate-50 dark:border-white/5'>
                    <div>
                      ...
                      <p className='font-black text-slate-900 dark:text-white'>
                        R$ {tenant.contract?.monthly_value.toLocaleString('pt-BR')}
                      </p>
                    </div>
                    <div className='text-right'>
                      ...
                      <p className='font-black text-slate-900 dark:text-white'>
                        Todo dia {tenant.contract?.payment_day}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {currentStep === 'profile' && (
            <div className='flex-1 flex flex-col animate-fadeIn'>
              <div className='p-8 pb-0'>
                <span className='px-3 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full text-[10px] font-black uppercase tracking-widest'>
                  Passo 02: Seu Cadastro
                </span>
                <h2 className='text-2xl font-black text-slate-900 dark:text-white mt-4 mb-2'>
                  Complete seus dados
                </h2>
                <p className='text-sm text-slate-500 font-medium mb-6'>
                  O proprietário solicitou os seguintes dados para conformidade.
                </p>
              </div>
            ...
            <div className='flex-1 flex flex-col animate-fadeIn'>
              <div className='p-8 pb-0'>
                <span className='px-3 py-1 bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 rounded-full text-[10px] font-black uppercase tracking-widest'>
                  Passo 03: Documentação
                </span>
                <h2 className='text-2xl font-black text-slate-900 dark:text-white mt-4 mb-2'>
                  Envie os documentos
                </h2>
                <p className='text-sm text-slate-500 font-medium mb-6'>
                  Suba fotos ou PDFs dos documentos obrigatórios.
                </p>
              </div>
              ...
              <div className='flex-1 px-8 space-y-4 overflow-y-auto'>
                {[
                  ...
                  .map((doc) => (
                    <div
                      key={doc.id}
                      className='p-4 bg-white dark:bg-surface-dark rounded-2xl border border-gray-100 dark:border-white/5 flex items-center justify-between group hover:border-primary/30 transition-all'
                    >
                      ...
                      <button className='text-[10px] font-black text-primary uppercase tracking-widest bg-primary/10 px-3 py-2 rounded-lg hover:bg-primary hover:text-white transition-all'>
                        Selecionar
                      </button>
                    </div>
                  ))}
            ...
            <div className='flex-1 flex flex-col animate-fadeIn'>
              <div className='p-8 pb-0'>
                <span className='px-3 py-1 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded-full text-[10px] font-black uppercase tracking-widest'>
                  Passo Final: Entrega do Imóvel
                </span>
                <h2 className='text-2xl font-black text-slate-900 dark:text-white mt-4 mb-2'>
                  Revise a Vistoria
                </h2>
                <p className='text-sm text-slate-500 font-medium mb-6'>
                  Confira o estado do imóvel no laudo de entrada preparado pelo proprietário.
                </p>
              </div>
                ...
                <div className='p-6 bg-white dark:bg-surface-dark border border-gray-100 dark:border-white/5 rounded-3xl flex flex-col items-center text-center space-y-4'>
                  ...
                  <div>
                    <h4 className='font-bold text-slate-900 dark:text-white'>
                      Laudo de Entrada Disponível
                    </h4>
                    <p className='text-xs text-slate-500 mt-1'>
                      Existem 12 itens para você revisar nos próximos dias.
                    </p>
                  </div>
          ...
          {currentStep === 'done' && (
            <div className='flex-1 flex flex-col items-center justify-center p-8 text-center space-y-8 animate-fadeIn'>
              ...
              <div>
                ...
                <p className='text-slate-500 dark:text-slate-400 font-medium max-w-sm mx-auto'>
                  Seu perfil está configurado e pronto para o uso. Você já pode acessar boletos,
                  contratos e falar com o proprietário.
                </p>
              </div>
              <div className='bg-blue-50 dark:bg-blue-900/10 p-4 rounded-2xl border border-blue-100 dark:border-blue-500/20 flex gap-3 text-left max-w-xs'>
                ...
                <p className='text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed'>
                  Sua pontualidade nos pagamentos e o cuidado com o imóvel aumentam seu{' '}
                  <strong>Score IGLOO</strong>.
                </p>
              </div>
              <button
                onClick={onComplete}

...
const AnnouncementsHistoryModal: React.FC<AnnouncementsHistoryModalProps> = ({
  isOpen,
  onClose,
  ownerId,
  onDuplicate,
  ...
  const [selectedAudience, setSelectedAudience] = useState<{ id: string; viewers: any[] } | null>(
    null
  );
  ...
  const fetchHistory = async () => {
    try {
      ...
      console.error('Error fetching history:', _error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm('Deseja realmente excluir este comunicado? Esta ação não pode ser desfeita.')
    )
      return;
    try {
  ...
  const getTargetLabel = (ann: OwnerAnnouncement) => {
    switch (ann.target_type) {
      case 'all':
        return 'Todos';
      case 'condominium':
        return ann.target_value?.name || 'Condomínio';
      case 'individual':
        return 'Individual';
      case 'property':
        return 'Imóvel';
  ...
  return (
    <div className='fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-6'>
      ...
      <div className='bg-white dark:bg-surface-dark w-full max-w-4xl h-[95vh] sm:h-[85vh] rounded-t-3xl sm:rounded-[32px] shadow-2xl border border-gray-100 dark:border-white/5 overflow-hidden animate-scaleUp z-10 flex flex-col'>
        {/* Header Section */}
        <div className='px-6 sm:px-10 py-6 sm:py-8 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-slate-50 dark:bg-black/20'>
          <div className='flex items-center gap-3 sm:gap-4'>
            ...
            <div>
              <h2 className='text-lg sm:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight'>
                Gestão
              </h2>
              <p className='text-[8px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-0.5 sm:mt-1'>
          ...
          <button
            onClick={onClose}
            className='w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center hover:bg-slate-100 dark:hover:bg-white/10 rounded-2xl transition-colors text-slate-400'
          >
                      ...
                      <div className='p-4 sm:p-5 flex items-center gap-3 sm:gap-4'>
                        ...
                        <div className='flex-1 min-w-0'>
                          <div className='flex flex-wrap items-center gap-1.5 mb-0.5 sm:mb-1'>
                            ...
                            <span
                              className={`text-[7px] sm:text-[8px] font-black uppercase tracking-widest px-1 py-0.5 rounded-md ${
                                isExpired
            ...
            <div className='px-6 sm:px-8 py-6 sm:py-8 border-b border-gray-100 dark:border-white/5 flex justify-between items-center'>
              <div className='flex items-center gap-3'>
                <button
                  ...
                  className='md:hidden w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-white/10 rounded-lg'
                >
                  <ChevronRight size={18} className='rotate-180' />
                </button>
                <h3 className='text-[10px] sm:text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest'>
                  Audiência
                </h3>
              </div>
              <button
                onClick={() => setSelectedAudience(null)}
                className='hidden md:block text-slate-400 hover:text-slate-900'
              >
                <X size={16} />
              </button>
            </div>

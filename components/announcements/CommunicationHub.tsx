...
const CommunicationHub: React.FC<CommunicationHubProps> = ({ onNewAnnouncement, onDuplicate }) => {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    ...
    try {
      const data = user.role === 'owner' 
        ...
        : await announcementService.getForTenant(user.id);
  ...
  return (
    ...
    >
      <div className='bg-white dark:bg-surface-dark h-full rounded-[24px] sm:rounded-[32px] p-4 sm:p-5 border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col relative overflow-hidden'>
        ...
        <div className='mb-3 sm:mb-4 relative z-20'>
          <p className='text-slate-500 dark:text-slate-400 text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] mb-1'>
            Governance Hub
          </p>
        ...
        <div className='flex-1 relative z-20 overflow-hidden min-h-[120px]'>
          <div className={`transition-all duration-1000 ${showPreviews ? 'opacity-0 -translate-y-4 scale-95 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className='pt-4 border-t border-gray-100 dark:border-white/5 mt-auto flex justify-between items-center relative z-20'>
          <button 

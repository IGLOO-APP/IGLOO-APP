...
export const AlertBadge = ({ icon: Icon, label, count, color, onClick }: AlertBadgeProps) => (
  ...
  >
    ...
    <div className='text-left flex-1 min-w-0'>
      <p className='text-[10px] font-semibold text-muted-foreground mb-0.5 truncate'>
        {label}
      </p>
      {count > 0 ? (
  ...
  return (
    <div className='flex justify-between items-center'>
      <div className='flex items-center gap-3'>
        {Icon && (
          <div
            className={`w-8 h-8 rounded-xl bg-muted flex items-center justify-center ${iconColor} shrink-0`}
          >
            <Icon size={18} />
          </div>
        )}
...
export const EmptyState = ({ icon: Icon, message, action, className = '' }: EmptyStateProps) => (
  <div className={`flex flex-col items-center justify-center py-6 sm:py-10 ${className}`}>
    ...
    <p className='text-xs font-medium text-muted-foreground mb-4 text-center max-w-xs'>
      {message}
    </p>
    {action && (
      <Button
...
export const PeriodSelector = ({ options, value, onChange }: PeriodSelectorProps) => (
  <Select value={value} onValueChange={onChange}>
    <SelectTrigger className='w-auto gap-2 pl-4 pr-3 py-2 rounded-2xl bg-muted border border-transparent text-xs font-medium text-muted-foreground hover:bg-accent transition-all'>
      <SelectValue />
    </SelectTrigger>

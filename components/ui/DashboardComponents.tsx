...
export const HeroCard = ({
  ...
  return (
    <Card
      className={`h-full p-4 active-tap transition-all duration-500 group relative flex flex-col justify-between ${
        variant === 'critical'
...
export const AlertBadge = ({ icon: Icon, label, count, color, onClick }: AlertBadgeProps) => (
  <Button
    onClick={onClick}
    variant='outline'
    className={`flex items-center gap-3 px-4 py-3 h-auto rounded-[32px] transition-all duration-300 active-tap shrink-0 ${color} bg-opacity-5 border-opacity-10 hover:bg-opacity-15 hover:border-opacity-20 shadow-sm hover:shadow-md group/alert`}
  >
        ...
        <div>
          <h2 className='text-base lg:text-lg font-black text-slate-900 dark:text-white flex items-center gap-2'>
            ...
            {tooltip && (
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger className='w-3.5 h-3.5 rounded-full bg-slate-200 dark:bg-white/10 text-slate-400 dark:text-slate-500 flex items-center justify-center text-[8px] font-bold leading-none cursor-help'>
                    ?
                  </TooltipTrigger>
...
export const PeriodSelector = ({ options, value, onChange }: PeriodSelectorProps) => (
  <Select value={value} onValueChange={onChange}>
    <SelectTrigger className='w-auto gap-2 pl-4 pr-3 py-2 rounded-2xl bg-slate-50 dark:bg-black/20 border border-transparent text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-white/5 transition-all'>
      <SelectValue />
      <ChevronDown size={12} className='text-slate-400' />
    </SelectTrigger>
    <SelectContent className='rounded-2xl border border-gray-100 dark:border-white/10 shadow-2xl'>
      {options.map((opt) => (
        <SelectItem
          key={opt.value}
          value={opt.value}

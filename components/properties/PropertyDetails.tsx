...
export const PropertyDetails: React.FC<PropertyDetailsProps> = ({ property, onClose }) => {
  ...
  return (
    <>
      <ModalWrapper
        onClose={onClose}
        className='md:max-w-2xl bg-background-light dark:bg-background-dark'
      >
        {/* Cover Image */}
        <div
          className='h-64 w-full bg-cover bg-center shrink-0 relative group'
        ...
        >
          ...
          <div className='absolute bottom-0 left-0 p-6 w-full'>
            <span
              className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-bold ring-1 ring-inset uppercase tracking-wide mb-2 bg-white/90 backdrop-blur-sm ${property.status_color?.replace('bg-', 'text-').replace('/10', '')}`}
            >
              {property.status}
            </span>
        ...
        <div className='flex-1 overflow-y-auto p-6 space-y-6 bg-background-light dark:bg-background-dark'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
              ...
              <p className='text-slate-900 dark:text-white text-xl font-bold mt-0.5'>
                {property.price}
              </p>
            </div>
            <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          ...
          {property.tenant ? (
            <div className='bg-white dark:bg-surface-dark rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden'>
              <div className='flex items-center justify-between mb-4'>
                ...
                <span className='bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide'>
                  Em dia
                </span>
              </div>
              <div className='flex items-center gap-4 mb-5'>
                <div
                  className='h-14 w-14 rounded-full bg-cover bg-center border-2 border-slate-100 dark:border-gray-700'
            ...
            <div className='bg-white dark:bg-surface-dark rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm'>
              ...
              <div className='space-y-3'>
                ...
                <div className='flex justify-between items-center py-2'>
                  <span className='text-slate-500 dark:text-slate-400 text-sm flex items-center gap-2'>
                    <DollarSign size={16} /> Valor Mensal
                  </span>
      ...
      {showInspections && (
        <PropertyInspection property={property} onClose={() => setShowInspections(false)} />
      )}

      {showDocuments && (
        <PropertyDocuments property={property} onClose={() => setShowDocuments(false)} />
      )}
    </>
  );
};

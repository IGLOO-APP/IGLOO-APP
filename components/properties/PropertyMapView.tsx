...
export const PropertyMapView: React.FC<PropertyMapViewProps> = ({ properties, onBack }) => {
  ...
  const visibleItems = useMemo(() => {
    ...
    if (activeLayers.includes('properties')) {
      items = [
        ...items,
        ...properties.map((p, i) => ({ ...p, type: 'property', ...getCoordinates(i) })),
      ];
    }
    ...
    if (searchTerm) {
      ...
      items = items.filter(
        (i) => i.name.toLowerCase().includes(lower) || i.address.toLowerCase().includes(lower)
      );
    }
  ...
  return (
    <div className='absolute inset-0 w-full h-full overflow-hidden bg-slate-100 dark:bg-slate-900 font-sans'>
      ...
      <div
        ...
        style={{
          backgroundImage:
            "url('https://lh3.googleusercontent.com/aida-public/AB6AXuBBg2hJrVlXyFhowT4lpA7MSKs1UeugFAUnJ8_twIuf6OCGT-hkYxI-nQt6fRMj3hFy0LyAsAZYPg21fYDplAUa9VYmvGJXJf9Q3tp7cOoBK9UB5amJYAvwsBR0Ts0S3aydWE7IWareyL59VwSLEMc2qJ3WkH7NCf5sJfC3P4p2m_6KYK83L6QCGvD6sSfPh-EhSbtAL-sb_gfXgHTLjbXMtQmdPRv3y14eQiyKtWvsaqsCglIjICh-MD00DnnKeTMS6wreJZxfG1O1')",
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              'radial-gradient(circle at 50% 50%, rgba(249, 115, 22, 0.25) 0%, transparent 40%), radial-gradient(circle at 25% 30%, rgba(239, 68, 68, 0.2) 0%, transparent 35%)',
        ...
        {visibleItems.map((item) => (
          ...
          >
            <div
              className={`relative flex items-center justify-center transition-transform duration-300 ${selectedId === item.id ? 'scale-125 z-20' : 'hover:scale-110'}`}
            >
              {selectedId === item.id && (
                <div
                  className={`w-12 h-12 rounded-full absolute animate-ping opacity-75 ${item.type === 'opportunity' ? 'bg-indigo-500/30' : 'bg-primary/30'}`}
                ></div>
              )}

              <div
                className={`w-10 h-10 rounded-full border-2 shadow-lg flex items-center justify-center transition-colors 
                        ${
      ...
      <div className='absolute right-4 top-40 z-20 flex flex-col gap-4 pointer-events-none'>
        ...
        >
          <Navigation
            size={22}
            className={scale !== 1 || position.x !== 0 ? 'text-primary fill-primary/20' : ''}
          />
        </button>
      </div>
      ...
      <div
        className={`absolute bottom-0 left-0 w-full z-30 flex flex-col justify-end pointer-events-none transition-transform duration-300 ease-out ${bottomSheetOpen ? 'translate-y-0' : 'translate-y-[110%]'}`}
      >
            ...
            <div className='px-5 pb-2'>
              <div className='flex flex-col gap-4'>
                <div className='flex gap-4'>
                  <div className='w-24 h-24 rounded-xl bg-slate-900 shrink-0 shadow-sm relative overflow-hidden group flex items-center justify-center'>
                    {selectedItem.image ? (
                      <div
                    ...
                    <div>
                      <div className='flex justify-between items-start'>
                        {selectedItem.type === 'opportunity' ? (
                          ...
                          <span
                            className={`inline-flex items-center rounded-md px-2 py-1 text-[10px] font-bold ring-1 ring-inset uppercase tracking-wide ${selectedItem.status_color?.replace('bg-', 'text-').replace('/10', '') || 'text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30'}`}
                          >
                            {selectedItem.status}
                          </span>
                        )}
                      </div>
                      <h3 className='mt-1 text-lg font-bold text-slate-900 dark:text-white leading-tight truncate pr-4'>
                        {selectedItem.name}
                      </h3>
                      <a
                ...
                <div className='grid grid-cols-3 gap-3'>
                  <div className='bg-slate-50 dark:bg-black/20 rounded-xl p-2.5 flex flex-col items-center justify-center border border-slate-100 dark:border-white/5'>
                    <span className='text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide'>
                      {selectedItem.type === 'opportunity' ? 'Investimento' : 'Aluguel'}
                    </span>
                  ...
                  <div className='bg-slate-50 dark:bg-black/20 rounded-xl p-2.5 flex flex-col items-center justify-center border border-slate-100 dark:border-white/5'>
                    <span className='text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide'>
                      {selectedItem.type === 'opportunity' ? 'Retorno Est.' : 'Contrato'}
                    </span>
                    <span
                      className={`text-sm font-bold mt-0.5 ${selectedItem.type === 'opportunity' ? 'text-emerald-500' : 'text-slate-900 dark:text-white'}`}
                    >
                  ...
                  <div className='bg-slate-50 dark:bg-black/20 rounded-xl p-2.5 flex flex-col items-center justify-center border border-slate-100 dark:border-white/5'>
                    <span className='text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide'>
                      {selectedItem.type === 'opportunity' ? 'Risco' : 'Próx. Venc'}
                    </span>
                    <span className='text-sm font-bold text-slate-900 dark:text-white mt-0.5'>

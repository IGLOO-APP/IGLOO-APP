        ...
        {view === 'detail' && (
          <div className='flex-1 flex flex-col overflow-hidden animate-fadeIn'>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    <div className='flex items-center gap-2 mb-1'>
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${isOwnerMode ? 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300' : 'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400'}`}
                      >
                        {isOwnerMode ? 'Análise de Divergências' : 'Ação Necessária'}
                      </span>
                      <span className='text-slate-400 dark:text-slate-500 text-xs font-medium'>
                        Vistoria de Saída #2024-03
                      </span>
                    </div>
            ...
            <div className='flex-1 overflow-y-auto p-4 md:p-6 space-y-8 bg-slate-50 dark:bg-black/10'>
              {/* Context Banner */}
              <div
                className={`${isOwnerMode ? 'bg-purple-50 border-purple-100 dark:bg-purple-900/10 dark:border-purple-800' : 'bg-blue-50 border-blue-100 dark:bg-blue-900/10 dark:border-blue-800'} border p-4 rounded-xl flex gap-3`}
              >
                {isOwnerMode ? (
                ...
                <div>
                  <h4
                    className={`font-bold text-sm ${isOwnerMode ? 'text-purple-800 dark:text-purple-300' : 'text-blue-800 dark:text-blue-300'}`}
                  >
                    {isOwnerMode ? 'Ação do Proprietário' : 'Instruções ao Inquilino'}
                  </h4>
                  <p
                    className={`text-xs mt-1 leading-relaxed ${isOwnerMode ? 'text-purple-700 dark:text-purple-400' : 'text-blue-700 dark:text-blue-400'}`}
                  >
                    {isOwnerMode
                ...
                <div className='flex items-center gap-4 pl-2'>
                  ...
                  <div>
                    <h4 className='font-bold text-slate-900 dark:text-white'>
                      Vistoria de Saída{' '}
                      <span className='text-red-500 text-[10px] uppercase bg-red-100 dark:bg-red-900/20 px-2 py-0.5 rounded ml-2'>
                        Divergência
                      </span>
                    </h4>
                    <div className='flex items-center gap-3 text-xs text-slate-500 mt-0.5'>
                      <span className='flex items-center gap-1'>
                        <Calendar size={12} /> 10 Mar 2024
                      </span>
                      <span className='flex items-center gap-1 font-medium text-slate-700 dark:text-slate-300'>
                        <MessageCircle size={12} /> Inquilino contestou 1 item
                      </span>
                    </div>
                  </div>
                </div>

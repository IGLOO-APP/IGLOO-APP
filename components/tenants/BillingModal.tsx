...
export const BillingModal: React.FC<BillingModalProps> = ({ tenant, onClose, onConfirm }) => {
  ...
  useEffect(() => {
    ...
    const formattedRent = editableRent.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL',
    });
    const formattedPenalty = penalty.toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL',
    });
  ...
  return (
    <ModalWrapper onClose={onClose} title='Cobrança Inteligente' className='md:max-w-xl'>

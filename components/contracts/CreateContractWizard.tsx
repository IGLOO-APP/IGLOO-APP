...
export const CreateContractWizard: React.FC<CreateContractWizardProps> = ({
  ...
  const getContractDataMap = () => {
    const selectedProp = properties.find(p => p.name === formData.property);
    return {
      // Template placeholders (Portuguese snake_case)
      ...
      numero_unidade: 'N/A',
      ocupacao_maxima: '2',
      data_inicio: formData.startDate ? new Date(formData.startDate).toLocaleDateString('pt-BR') : '',
      ...
      valor_taxa_rateio: formData.hasMaintenanceFee && formData.maintenanceFee ? formData.maintenanceFee : '0,00 (Não aplicável)',
  ...
  const handleNext = () => {
    ...
    if (currentStep < 6) {
      ...
      // Page dimensions used for the simulated A4 view
      const TOTAL_WIDTH = 850;
      const TOTAL_HEIGHT = 1030;

      // Convert every signature's pixel position to viewport-independent percentages
      ...
      onComplete({
        ...
        uploadedFile,
        docMode,
        signaturePayloads, // structured for API storage & PDF stamping
      });
    }
  };
  ...
  const handleSignatureConfirm = (dataUrl: string) => {
    ...
    // Posição inicial segura (canto inferior direito adaptado para a página real)
    setSignaturePositions((prev) => ({
      ...prev,
      [lastPageIndex]: { x: 650, y: 900 }
    }));

    setShowSignatureModal(false);
    setMovingSignature(lastPageIndex); // Ativa o modo de movimentação
  };
  ...
  return (
    <div className='fixed inset-0 z-50 bg-slate-50 dark:bg-background-dark flex flex-col animate-slideUp overflow-hidden'>
          ...
          <div className='hidden md:flex items-center gap-2'>
            {[
              ...
              <React.Fragment key={step.n}>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all ${currentStep === step.n
          ...
          <div className='flex items-center gap-3 min-w-[200px] justify-end'>
            ...
            <div className='relative group'>
              ...
              <button
                onClick={handleNext}
                disabled={!canAdvance()}
                className={`px-5 py-2 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg transition-all flex items-center gap-2 ${!canAdvance()
      ...
      <div className='flex-1 overflow-y-auto bg-slate-50 dark:bg-black/20'>
        <div
          className={`mx-auto min-h-full flex flex-col transition-all duration-300 ${currentStep === 5 ? 'max-w-[1600px] w-full p-0' : 'max-w-4xl p-6 md:p-10'
            }`}
        >
          {currentStep === 1 && (
                ...
                {loading ? (
                  ...
                  properties.map((prop) => (
                    <button
                      ...
                      className={`group relative overflow-hidden rounded-3xl border-2 text-left transition-all duration-300 flex flex-col bg-white dark:bg-surface-dark ${formData.property === prop.name
                        ...
                        tenants
                          ...
                          .map((tenant) => (
                            <button
                              ...
                              className={`group p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-4 ${selectedTenantId === tenant.id
                  ...
                  <div className='bg-white dark:bg-surface-dark p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-white/5 space-y-8'>
                    <div>
                      <div className='flex items-center justify-between mb-2 ml-1'>
                        ...
                        {formData.rentValue !== properties.find(p => p.name === formData.property)?.price.replace(/[^\d,]/g, '').replace(',', '.') && (
                          <button
                            onClick={() => {
                              const base = properties.find(p => p.name === formData.property)?.price.replace(/[^\d,]/g, '').replace(',', '.') || '0';
                    ...
                    <div className='pt-6 border-t border-slate-100 dark:border-white/5'>
                      <div className='flex items-center justify-between mb-4'>
                        <div className='flex flex-col'>
                          ...
                          <span className='text-[10px] text-slate-400 font-bold'>Para kitnets sem condomínio formal</span>
                        </div>
                        <button
                          type='button'
                          onClick={() => setFormData({ ...formData, hasMaintenanceFee: !formData.hasMaintenanceFee })}
                        ...
                        >
                          <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all duration-300 shadow-md ${formData.hasMaintenanceFee ? 'left-6' : 'left-1'}`} />
                        </button>
                      </div>
                      ...
                      {formData.hasMaintenanceFee && (
                        <div className='space-y-4 animate-fadeIn'>
                          ...
                          <div className='p-4 bg-primary/5 dark:bg-primary/10 rounded-2xl border border-primary/20'>
                            <p className='text-xs text-slate-500 leading-relaxed font-medium'>
                              Rateio mensal fixo cobrado por você para cobrir limpeza de corredores, internet coletiva, água compartilhada e demais serviços.
                            </p>
                          </div>
                        </div>
                      )}
                ...
                <div className='md:col-span-3 space-y-6'>
                  <div className='bg-white dark:bg-surface-dark p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-white/5 space-y-8'>
                      ...
                      <div className='grid grid-cols-3 gap-3'>
                        {[12, 30, 36].map((m) => (
                          <button
                            ...
                            className={`py-4 rounded-2xl border-2 font-black transition-all ${formData.duration === m.toString()
                    ...
                    <div
                      ...
                      className={`relative group flex items-center gap-4 p-6 rounded-3xl border-2 cursor-pointer transition-all overflow-hidden ${formData.autoRenew
                    ...
                    >
                      ...
                      <div
                        className={`w-12 h-12 rounded-2xl border-2 flex items-center justify-center transition-all ${formData.autoRenew
                    ...
                    <div className='pt-6 border-t border-slate-100 dark:border-white/5'>
                      ...
                      <div className='grid grid-cols-3 gap-3'>
                        {[1, 2, 3].map((m) => (
                          <button
                            key={m}
                            onClick={() => setFormData({ ...formData, earlyTerminationFee: m.toString() })}
                            className={`py-4 rounded-2xl border-2 font-black transition-all relative overflow-hidden ${formData.earlyTerminationFee === m.toString()
                                ? 'border-primary bg-primary/5 text-primary scale-[1.02] shadow-md'
                                : 'border-slate-50 dark:border-white/5 bg-slate-50 dark:bg-black/20 text-slate-400 hover:border-slate-200'
                              }`}
                          >
                            ...
                            {parseInt(formData.duration) <= 12 && m === 1 && (
                              <span className='absolute top-1 right-1 text-[7px] font-black bg-amber-500 text-white px-1.5 py-0.5 rounded-full uppercase tracking-widest'>Short-stay</span>
                            )}
                          </button>
                        ))}
                      </div>
                      {parseInt(formData.duration) <= 12 && (
                        <div className='mt-3 p-3 bg-amber-500/5 rounded-2xl border border-amber-500/20 flex items-center gap-2'>
                          ...
                          <p className='text-[10px] text-amber-700 dark:text-amber-400 font-bold'>Contratos curtos (≤12 meses): recomendamos multa de 1 mês, proporcional ao Art. 4º da Lei 8.245/91.</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className='pt-6 border-t border-slate-100 dark:border-white/5'>
                      ...
                      <div className='grid grid-cols-3 gap-3'>
                        {[3, 6, 12].map((m) => (
                          <button
                            key={m}
                            onClick={() => setFormData({ ...formData, lockInPeriod: m.toString() })}
                            className={`py-4 rounded-2xl border-2 font-black transition-all ${formData.lockInPeriod === m.toString()
                                ? 'border-primary bg-primary/5 text-primary scale-[1.02] shadow-md'
                                : 'border-slate-50 dark:border-white/5 bg-slate-50 dark:bg-black/20 text-slate-400 hover:border-slate-200'
                              }`}
                          >
                      ...
                      <div className='mt-3 p-3 bg-slate-50 dark:bg-black/30 rounded-2xl border border-slate-100 dark:border-white/5'>
                        <p className='text-[10px] text-slate-500 font-bold leading-relaxed'>Período mínimo em que a multa rescisória é cobrada integralmente, sem proporcionalidade. Protege o investimento de reforma entre inquilinos.</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {currentStep === 5 && (
            <div className='flex-1 flex flex-col lg:flex-row items-stretch bg-slate-100 dark:bg-black/40 p-6 gap-6 overflow-y-auto animate-fadeIn'>
              ...
              {/* COLUNA DA ESQUERDA: RENDERIZADOR DE PÁGINAS (ESTILO DOCUMENTO REAL) */}
              <div className='flex-1 flex flex-col items-center gap-10 overflow-y-auto pr-2'>
                {contractPages.map((pageContent, index) => (
                  <div
                    ...
                    style={{ height: '1080px' }} // Força tamanho estrito A4 proporcional
                  >
                    {/* Cabeçalho interno da página decorativo */}
                    <div className='absolute top-6 left-16 right-16 flex justify-between items-center text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest pointer-events-none select-none border-b border-slate-100 dark:border-white/5 pb-2 z-0'>
                      ...
                      <span>Página {index + 1} de {contractPages.length}</span>
                    </div>

                    {/* TEXTAREA BLINDADO (CORREÇÃO 2) */}
                    <textarea
                      ref={(el) => { textareaRefs.current[index] = el; }}
                      value={pageContent}
                      onChange={(e) => updatePageContent(index, e.target.value)}
                      style={{
                        // Se houver assinatura ativa nesta página ou se o usuário estiver movendo ela, 
                        // liberamos o ponteiro para gerenciar o drag sem interceptação do editor de texto
                        pointerEvents: signatures[index] ? 'none' : 'auto'
                      }}
                    ...
                    {/* COMPONENTE DE DRAG DA ASSINATURA (CORREÇÃO 3 COMPLETA) */}
                    {signatures[index] && signaturePositions[index] && (
                      <div
                        ...
                        onMouseDown={(e) => {
                          ...
                          setMovingSignature(index); // Mantém o estado visual ativo
                          ...
                          const onMouseMove = (moveEvent: MouseEvent) => {
                            ...
                            setSignaturePositions(prev => ({
                              ...prev,
                              [index]: {
                                // Limita o movimento dentro das margens da página A4 (~850x1030)
                          ...
                          const onMouseUp = () => {
                            ...
                            // Não limpamos o movingSignature aqui imediatamente para permitir 
                            // que o usuário saiba qual assinatura ele acabou de ajustar.
                      ...
                      >
                        {/* Tooltip de Ajuda ao Usuário */}
                        <div className='absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] font-black px-2 py-1 rounded-md flex items-center gap-1 whitespace-nowrap shadow-md pointer-events-none opacity-90 transition-opacity'>
                          <Move size={10} /> Arraste para posicionar
                        </div>

                        {/* Imagem da Assinatura Gerada */}
                        <img
                    ...
                    {/* BLOCO DE ASSINATURAS DO FECHAMENTO (ÚLTIMA PÁGINA - CORREÇÃO 6) */}
                    {index === contractPages.length - 1 && (
                      <div className='mt-auto p-16 pt-0 grid grid-cols-2 gap-12 relative z-20'>
                        ...
                        <div
                          ...
                          onClick={() => {
                            // Se o usuário clicar aqui, abre o modal e já seta a posição perfeita automaticamente
              ...
              {/* COLUNA DA DIREITA: CONTROLES E STATUS (MANTIDO DO SEU CÓDIGO) */}
              <div className='w-full lg:w-[380px] p-6 bg-white dark:bg-surface-dark rounded-2xl border border-slate-200 dark:border-white/5 shrink-0 flex flex-col justify-between shadow-xl h-fit sticky top-6'>
                <div className='space-y-6'>
                  <div>
                    <span className='px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[9px] font-black uppercase tracking-widest block w-fit mb-2'>Assinatura Digital</span>
                    <h4 className='text-lg font-black text-slate-900 dark:text-white tracking-tight uppercase'>Gestão de Firmas</h4>
                    <p className='text-xs text-slate-500 font-medium mt-1'>Arraste a sua rubrica eletrônica para o local adequado em cada folha ou clique na área demarcada ao final.</p>
                  </div>

                  <button
                    onClick={() => {
                      // Se já existir assinatura, permite clicar para reabrir o modal e reajustar se necessário
                      setShowSignatureModal(true);
                    }}
                    className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-all active:scale-95 shadow-md ${Object.keys(signatures).length > 0
                        ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-500/10'
                        : 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 shadow-slate-900/10'
                      }`}
                  >
                    <PenTool size={16} /> {Object.keys(signatures).length > 0 ? 'Assinatura Coletada ✓' : 'Inserir Nova Firma'}
                  </button>

                  {movingSignature !== null && (
                    <button
                      onClick={() => setMovingSignature(null)}
              ...
              <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-[1200px] mx-auto'>
                ...
                <div className='lg:col-span-2 space-y-6'>
                  ...
                  <div className='bg-white dark:bg-surface-dark p-8 rounded-[40px] border border-slate-100 dark:border-white/5 shadow-2xl'>
                    ...
                    {/* Kitnet-specific details row */}
                    <div className='grid grid-cols-2 md:grid-cols-3 gap-8 mt-6 pt-6 border-t border-slate-100 dark:border-white/5'>
                      <div className='space-y-1'>
                        <span className='text-[10px] font-black text-slate-400 uppercase tracking-widest block'>Multa Rescisória</span>
                        <p className='text-xl font-black text-slate-900 dark:text-white leading-none'>{formData.earlyTerminationFee}x Aluguel</p>
                        <span className='text-[10px] font-bold text-slate-400'>Proporcional (Art. 4º)</span>
                      </div>
                      <div className='space-y-1'>
                        <span className='text-[10px] font-black text-slate-400 uppercase tracking-widest block'>Lock-in</span>
                        <p className='text-xl font-black text-slate-900 dark:text-white leading-none'>{formData.lockInPeriod} Meses</p>
                      ...
                      {formData.hasMaintenanceFee && formData.maintenanceFee && (
                        <div className='space-y-1'>
                          <span className='text-[10px] font-black text-slate-400 uppercase tracking-widest block'>Taxa de Rateio</span>
                          <p className='text-xl font-black text-primary leading-none'>R$ {formData.maintenanceFee}</p>
        ...
        <div className='fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn'>
          <div className='bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden border border-slate-200 dark:border-white/10'>
            <div className='p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between'>
              <div>
                <h3 className='text-lg font-black text-slate-900 dark:text-white uppercase tracking-tighter'>Pré-visualização da Minuta</h3>
                <p className='text-xs text-slate-500'>Visualize a versão final formatada para impressão</p>
              </div>
              <button
            ...
            <div className='flex-1 bg-slate-100 dark:bg-slate-950 p-4'>
              <PDFViewer width="100%" height="100%" style={{ border: 'none', borderRadius: '12px' }}>
                <ContractPDFTemplate
                  data={getContractDataMap()}
                />
              </PDFViewer>
            </div>
          </div>

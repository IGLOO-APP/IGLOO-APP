...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const navigationItems = [
    {
      id: 'nav-dashboard',
      title: 'Painel Geral',
      subtitle: 'Visão geral do seu patrimônio',
    ...
    },
    {
      id: 'nav-properties',
      title: 'Meus Imóveis',
      subtitle: 'Gestão de ativos e unidades',
      icon: Building2,
      path: '/properties',
      type: 'Página',
    },
    ...
    },
    {
      id: 'nav-financials',
      title: 'Finanças',
      subtitle: 'Fluxo de caixa e cobranças',
      icon: Receipt,
      path: '/financials',
      type: 'Página',
    },
  ...
  return (
    <div className='fixed inset-0 z-[100] flex items-start justify-center pt-20 px-4 md:pt-[10%]'>
      <div
        className='fixed inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm animate-fadeIn'
        onClick={onClose}
      />
      ...
      <div className='relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden animate-scaleUp'>
        ...
        <div className='flex items-center px-6 py-5 border-b border-slate-100 dark:border-white/5'>
          ...
          <div className='flex items-center gap-2'>
            <span className='hidden sm:flex px-2 py-1 bg-slate-100 dark:bg-white/5 rounded-md text-[10px] font-black text-slate-400 uppercase tracking-widest'>
              ESC
            </span>
            <button
              onClick={onClose}
          ...
          {filteredItems.length > 0 ? (
                ...
                >
                  <div className='flex items-center gap-4 min-w-0'>
                    ...
                    <div className='text-left min-w-0'>
                      <div className='flex items-center gap-2'>
                        <span className='text-slate-900 dark:text-white font-black text-sm tracking-tight truncate'>
                          {item.title}
                        </span>
                        {item.type && (
                          <span className='px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 text-[8px] font-black uppercase text-slate-400 tracking-widest border border-slate-200 dark:border-white/5'>
                            {item.type}
                          </span>
                        )}
                      </div>
            ...
            <div className='py-12 flex flex-col items-center justify-center text-center'>
              ...
              <p className='text-sm text-slate-500 dark:text-slate-400'>
                Tente usar outras palavras-chave ou navegue pelo menu.
              </p>
            </div>
          )}
        </div>
          ...
          <div className='flex items-center gap-6'>
            <div className='flex items-center gap-2'>
              <div className='flex items-center gap-1'>
                <span className='px-1.5 py-0.5 bg-white dark:bg-white/10 rounded shadow-sm text-[9px] font-bold text-slate-500'>
                  ↑
                </span>
                <span className='px-1.5 py-0.5 bg-white dark:bg-white/10 rounded shadow-sm text-[9px] font-bold text-slate-500'>
                  ↓

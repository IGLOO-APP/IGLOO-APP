          ...
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-sm font-bold text-slate-900 dark:text-white capitalize flex items-center gap-2'>
                ...
                {card.is_default && (
                  <span className='text-[10px] bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded text-slate-500 uppercase tracking-wider'>
                    Padrão
                  </span>
                )}
              </p>
              <p className='text-xs text-slate-500'>
                Expira em {card.exp_month}/{card.exp_year}
              </p>
            </div>
          </div>

          <div className='flex items-center gap-2'>
            {selectedId === card.id && <CheckCircle size={20} className='text-primary' />}
            {onDelete && (
              <button
                onClick={(e) => {
                  e.stopPropagation();

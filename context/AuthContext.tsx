...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (isLoaded && session) {
      ...
      // Atualiza o token a cada 55 segundos para evitar expiração (o JWT do Clerk para Supabase expira em 1 min)
      const intervalId = setInterval(updateSupabaseToken, 55000);
    ...
    try {
      ...
      if (profile) {
        const userData: User = {
          ...
          role:
            ((profile as any).admin_type ? 'admin' : (profile.role as UserRole)) ||
      ...
      if (clerkUser) {
        setUser({
          id: clerkUser.id,
          name:
            `${clerkUser.firstName || ''} ${clerkUser.lastName || ''}`.trim() || 'Usuário Igloo',
          email: clerkUser.primaryEmailAddress?.emailAddress || '',

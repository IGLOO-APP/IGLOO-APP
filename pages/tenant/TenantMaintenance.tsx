...
const TenantMaintenance: React.FC = () => {
  ...
  const { data: allMessages = [] } = useQuery({
    queryKey: [
      'tenant_all_maintenance_messages',
      currentUser?.id,
  ...
  const activeTicketMessages = activeTicket
    ? allMessages.filter(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (m: any) => m.requestId === activeTicket.id || m.ticketId === activeTicket.id
      )
    : [];
  ...
  const displayRequests = [
    ...(pendingTicket && !requests.find((r) => r.id === pendingTicket.id) && activeTab === 'open'
      ? [pendingTicket]
      : []),
    ...filteredRequests,
  ];
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (content: string) => {
      ...
      return tenantService.sendMaintenanceMessage(
        activeTicket.id,
        ...
        <div className='p-6 space-y-6 shrink-0'>
          ...
          <div className='flex p-1 bg-white/5 rounded-xl'>
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'open' ? 'bg-white/10 text-white' : 'text-slate-500'}`}
            >
              Em Aberto
            </button>
            <button
              onClick={() => setActiveTab('closed')}
              className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'closed' ? 'bg-white/10 text-white' : 'text-slate-500'}`}
            >
              Concluídos
            </button>
          </div>
          ...
          <div className='relative'>
            ...
            <input
              ...
              className='w-full h-10 pl-9 pr-4 rounded-xl bg-white/5 border-none text-xs font-bold text-white placeholder-slate-700 focus:ring-1 focus:ring-primary/30'
            />
          </div>
        </div>
                ...
                >
                  <div className='max-w-sm w-full bg-white/[0.03] border border-white/5 rounded-3xl p-5 space-y-3 backdrop-blur-sm shadow-2xl'>
                    <div className='flex items-center justify-between'>
                      ...
                      <span
                        className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border ${getStatusStyle(msg.ticketStatus || 'open')}`}
                      >
                        {msg.ticketStatus}
                      </span>
                    </div>
                    <div>
                      ...
                      <p className='text-[11px] text-slate-500 font-medium mt-1 leading-relaxed line-clamp-2 italic'>
                        "{msg.content}"
                      </p>
                    </div>
          ...
          {allMessages.length === 0 && !isCreating && (
            <div className='flex flex-col items-center justify-center min-h-[350px] my-auto text-center animate-fadeIn'>
              ...
              <p className='text-xs font-medium text-slate-500 max-w-sm leading-relaxed mb-6'>
                Você não possui nenhum chamado de manutenção em aberto no momento. Precisa de ajuda
                com alguma coisa no seu imóvel?
              </p>
              <button
                onClick={() => {
                  setIsCreating(true);
            ...
            <div className='max-w-md mx-auto lg-card lg-card-lift p-6 text-center space-y-4 my-8 animate-fadeIn'>
              ...
              <div>
                <h4 className='text-sm font-black text-white uppercase tracking-wider'>
                  Todos os chamados concluídos
                </h4>
                <p className='text-[11px] text-slate-500 font-medium mt-1 leading-relaxed'>
                  Você não possui nenhum chamado ativo. Caso precise de alguma nova manutenção, abra
                  um novo chamado abaixo.
                </p>
              </div>
              <button
                onClick={() => {

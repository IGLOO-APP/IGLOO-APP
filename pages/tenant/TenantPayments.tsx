...
const TenantPayments: React.FC = () => {
  ...
  const totalPaidCurrentYear = payments
    .filter(
      (p) => p.status === 'paid' && new Date(p.due_date).getFullYear() === new Date().getFullYear()
    )
                ...
                <div className='flex items-center gap-2'>
                  <div className='h-2 flex-1 bg-white/20 rounded-full overflow-hidden'>
                    <div
                      ...
                      style={{
                        width: `${totalPaidPaymentsCount > 0 ? (onTimePaymentsCount / totalPaidPaymentsCount) * 100 : 0}%`,
                      }}
                    ></div>
                  </div>
                  <span className='text-xs font-bold'>
                ...
                <div className='space-y-1'>
                  ...
                  {payment.breakdown?.fine && (
                    <p className='text-xs text-red-500 flex justify-between w-32 font-bold'>
                      <span>Multa/Juros:</span> <span>R$ {payment.breakdown.fine.toFixed(2)}</span>
                    </p>
                  )}
                </div>

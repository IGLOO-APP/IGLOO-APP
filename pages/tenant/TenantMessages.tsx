...
const TenantMessages: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: 'Olá João, gostaria de agendar a vistoria anual do imóvel para a próxima semana.',
      sender: 'owner',
      time: '09:30',
    },
    ...
    {
      id: 3,
      text: 'Perfeito. Quinta às 14h. Te aviso quando estiver chegando.',
      sender: 'owner',
      time: '09:34',
    },
    ...
    <div className='flex flex-col h-full w-full max-w-md mx-auto md:max-w-4xl bg-background-light dark:bg-background-dark relative'>
      ...
      <div className='flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-black/20'>
        <div className='flex justify-center mb-4'>
          <span className='text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-white/5 px-3 py-1 rounded-full'>
            Hoje
          </span>
        </div>
      ...
      <div className='p-4 bg-surface-light dark:bg-surface-dark border-t border-gray-200 dark:border-white/5 shrink-0'>
        <form onSubmit={handleSendMessage} className='flex gap-3 items-end'>
          <button
            type='button'
            className='p-3 text-slate-400 hover:text-primary transition-colors hover:bg-slate-100 dark:hover:bg-white/5 rounded-full'
          >
            <Paperclip size={20} />
          </button>

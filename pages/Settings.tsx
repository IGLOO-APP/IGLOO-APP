  ...
  return (
    <div className='flex flex-col h-full w-full max-w-md mx-auto md:max-w-5xl relative bg-background-light dark:bg-background-dark'>
      ...
      <div className='flex flex-col md:flex-row h-full overflow-hidden'>
        {/* Navigation */}
        <aside className='shrink-0 md:w-64 space-y-1.5 p-4 md:p-0 md:mr-8'>
          {[
            { id: 'general', label: user?.role === 'tenant' ? 'Perfil' : 'Perfil & Empresa', icon: User },
            ...((user?.role === 'owner' || user?.role === 'admin')
              ? [
                  { id: 'financial', label: 'Financeiro', icon: CreditCard },
                  { id: 'subscription', label: 'Assinatura', icon: Crown, highlight: true },
                ...
                <div className='relative z-10 flex flex-col md:flex-row justify-between items-center gap-6'>
                  <div>
                    ...
                    <p className='text-indigo-100 text-sm mt-2 max-w-md'>
                      Conecte sua conta bancária para receber pagamentos de aluguéis automaticamente
                      via Cartão e Pix com split de taxas.
                    </p>
                  </div>
                  {stripeConnected ? (
                    ...
                    <div className='p-5'>
                      <div className='flex items-start justify-between'>
                        ...
                        <div className='flex items-center gap-2'>
                          <button
                            onClick={() =>
                              setExpandedMethodId(expandedMethodId === method.id ? null : method.id)
                            }
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                      ...
                      <div>
                        <div className='flex justify-between text-xs mb-2 font-bold text-slate-700 dark:text-slate-300'>
                          ...
                          <span>
                            ...
                            {currentPlanDetails.limits.properties === -1
                              ? '∞'
                              : currentPlanDetails.limits.properties}
                          </span>
                        </div>
                        <div className='w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden'>
                          <div
                            ...
                            style={{
                              width: `${currentPlanDetails.limits.properties === -1 ? 5 : (subscription.usage.properties / currentPlanDetails.limits.properties) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className='flex justify-between text-xs mb-2 font-bold text-slate-700 dark:text-slate-300'>
                          ...
                          <span>
                            {subscription.usage.tenants} /{' '}
                            {currentPlanDetails.limits.tenants === -1
                              ? '∞'
                              : currentPlanDetails.limits.tenants}
                          </span>
                        </div>
                        <div className='w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden'>
                          <div
                            ...
                            style={{
                              width: `${currentPlanDetails.limits.tenants === -1 ? 5 : (subscription.usage.tenants / currentPlanDetails.limits.tenants) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className='flex justify-between text-xs mb-2 font-bold text-slate-700 dark:text-slate-300'>
                          ...
                          <span>
                            {subscription.usage.storage_used_gb} GB /{' '}
                            {currentPlanDetails.limits.storage_gb} GB
                          </span>
                        </div>
                        <div className='w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden'>
                          <div
                            ...
                            style={{
                              width: `${(subscription.usage.storage_used_gb / currentPlanDetails.limits.storage_gb) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
            ...
            {checkoutStep === 'success' && (
              <div className='py-10 flex flex-col items-center justify-center text-center animate-scaleUp'>
                ...
                <p className='text-slate-500 max-w-xs mx-auto mt-2 mb-8'>
                  Sua assinatura do plano <strong>{checkoutPlanDetails.name}</strong> foi
                  confirmada. Você já tem acesso a todos os recursos.
                </p>
                <button
                  onClick={() => {

...
import {
  DollarSign, Square, Bed, Bath, Car, Clock, Eye, User, Phone, Mail,
  FileText, Calendar, CheckCircle, Edit2, Trash2, TrendingUp, AlertCircle,
  Award, Hash, CircleDollarSign, MessageSquare, Loader2, ShieldCheck,
} from 'lucide-react';
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 gap-3'>
        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            ...
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Aluguel</p>
          </div>
        ...
        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            ...
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Área</p>
          </div>
        ...
        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            ...
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Quartos</p>
          </div>
          <p className='text-slate-900 dark:text-white text-lg font-black'>{property.bedrooms ?? 0}</p>
        </div>

        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            <Bath size={14} className='text-primary' />
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Banheiros</p>
          </div>
          <p className='text-slate-900 dark:text-white text-lg font-black'>{property.bathrooms ?? 0}</p>
        </div>

        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            <Car size={14} className='text-primary' />
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Vagas</p>
          </div>
          <p className='text-slate-900 dark:text-white text-lg font-black'>{property.parking ?? 0}</p>
        </div>

        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            <Clock size={14} className='text-primary' />
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>{timeInfo.sub}</p>
          </div>
        ...
        <div className='bg-white dark:bg-surface-dark p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
          <div className='flex items-center gap-2 mb-1'>
            ...
            <p className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest'>Visitas</p>
          </div>
      ...
      {tenant ? (
        <div className='bg-white dark:bg-surface-dark rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden'>
          ...
          <div className='flex items-center gap-4 mb-5 cursor-pointer' onClick={navigateToTenant}>
            ...
            />
            <div>
              <p className='text-lg font-black text-slate-900 dark:text-white leading-tight'>{tenant.name}</p>
        ...
        <div className='bg-white dark:bg-surface-dark rounded-2xl p-8 border border-dashed border-gray-300 dark:border-gray-700 shadow-sm flex flex-col items-center justify-center text-center'>
          ...
          <p className='text-slate-500 dark:text-slate-400 text-xs mt-1 mb-6 max-w-[200px]'>Este imóvel está disponível para locação.</p>
        ...
        <div className='bg-white dark:bg-surface-dark rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm'>
          ...
          <div className='space-y-4'>
            <div className='flex justify-between items-center pb-3 border-b border-gray-50 dark:border-gray-800'>
              ...
              <span className='text-slate-900 dark:text-white font-bold text-sm'>{contract.start_date}</span>
            </div>
            <div className='flex justify-between items-center pb-3 border-b border-gray-50 dark:border-gray-800'>
              <span className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-2'>
                <CheckCircle size={14} /> Término
              </span>
              <span className='text-slate-900 dark:text-white font-bold text-sm'>{contract.end_date}</span>
            </div>
            <div className='flex justify-between items-center'>
              <span className='text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-2'>
                <DollarSign size={14} /> Valor Mensal
              </span>
              <span className='text-slate-900 dark:text-white font-black text-sm'>{contract.value}</span>
            </div>
          </div>
        </div>
      )}
                ...
                <div className='space-y-6'>
                  <div>
                    ...
                    <div className='flex items-center gap-4'>
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${isLate ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                      ...
                      <div>
                        <div className='text-lg font-black text-slate-900 dark:text-white leading-none mb-1'>
                          {loadingPayments ? <Loader2 size={16} className='animate-spin' /> : paymentScore}
                        </div>
                  ...
                  <div className='p-6 bg-slate-50 dark:bg-black/10 rounded-3xl border border-slate-100 dark:border-white/5'>
                    ...
                    <div className='w-full h-1.5 bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden'>
                      <div className='h-full bg-primary rounded-full transition-all duration-1000' style={{ width: `${trustLevel}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-6'>
                  <div className='flex items-start gap-4'>
                    ...
                    <div>
                      ...
                      <span className='text-sm font-bold text-slate-700 dark:text-slate-200'>{contract?.start_date || 'N/A'}</span>
                    </div>
                  </div>
                  <div className='flex items-start gap-4'>
                    <div className='w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400'>
                    ...
                    <div>
                      ...
                      <span className='text-sm font-bold text-slate-700 dark:text-slate-200'>{contract ? `${Math.ceil((new Date().getTime() - new Date(contract.start_date).getTime()) / (1000 * 60 * 60 * 24 * 30))} Meses (Ativo)` : 'N/A'}</span>
                    </div>
                  </div>
                  <div className='flex items-start gap-4'>
                    <div className='w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400'>

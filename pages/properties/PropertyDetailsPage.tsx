...
import { ArrowLeft, TrendingUp, ClipboardCheck, FileText, User, ExternalLink, Maximize2, ChevronLeft, ChevronRight, MapPin, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className='flex flex-col h-full bg-background-light dark:bg-background-dark overflow-hidden transition-colors duration-300'>
      ...
      <div className='flex-1 overflow-y-auto p-8 pt-4 pb-32'>
          ...
          <div className='h-64 w-full relative group shrink-0 overflow-hidden bg-slate-900 mb-6 rounded-[32px] shadow-sm'>
            {h.hasImages ? (
              ...
              <div className='absolute inset-0 flex items-center justify-center bg-slate-900'>
                <span className='text-white/10 text-4xl font-black uppercase tracking-[0.4em] select-none'>Meu Igloo</span>
              </div>
            )}
            ...
            {h.images.length > 1 && (
              <div className='absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-3 py-2 bg-black/30 backdrop-blur-xl rounded-2xl border border-white/10 z-20 shadow-2xl'>
                <button onClick={h.prevImage} className='p-1.5 rounded-xl hover:bg-white/10 text-white transition-all active:scale-90'>
                  <ChevronLeft size={18} />
                </button>
                <div className='flex gap-1.5 px-1'>
                  {h.images.map((_, i) => (
                    <div key={i} className={`h-1 rounded-full transition-all duration-300 ${i === h.currentImageIndex ? 'w-4 bg-primary' : 'w-1 bg-white/30'}`} />
                  ))}
                </div>
                <button onClick={h.nextImage} className='p-1.5 rounded-xl hover:bg-white/10 text-white transition-all active:scale-90'>
            ...
            <div className='absolute bottom-0 left-0 p-6 w-full z-10 bg-gradient-to-t from-black/80 via-transparent to-transparent pt-20'>
              ...
              <div className='flex items-end justify-between gap-4'>
                <div className='flex-1'>
                  <h2 className='text-3xl font-black text-white leading-tight drop-shadow-lg'>{property.name}</h2>
        ...
        {h.activeTab === 'docs' && <DocsTab property={property} />}

...
const Login: React.FC = () => {
  ...
  const handleEmailStep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        clerkError?.longMessage || clerkError?.message || err.message || 'E-mail não encontrado.';
  ...
  const handlePasswordStep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const clerkError = err.errors?.[0];
      const message =
        clerkError?.longMessage || clerkError?.message || err.message || 'Senha incorreta.';
      setError(String(message));
    } finally {
      setLoading(false);
    }
  ...
  const handleCodeStep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const clerkError = err.errors?.[0];
      const message =
        clerkError?.longMessage || clerkError?.message || err.message || 'Código inválido.';
      setError(String(message));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AuthLayout title='Login' subtitle='Bem-vindo à Igloo Platform'>
      <div className='animate-auth-entry'>
          ...
          {step === 'email' && (
            <>
              ...
              <p className='text-slate-400 text-sm font-medium'>
                Use seu e-mail corporativo ou social
              </p>
            </>
          )}
          {step === 'password' && (
            <>
              ...
              <p className='text-slate-400 text-sm font-medium truncate'>
                Acessando como{' '}
                <span className='text-white border-b border-primary/30'>
          ...
          {step === 'code' && (
            <>
              ...
              <p className='text-slate-400 text-sm font-medium'>
                ...
                <span className='text-white'>{typeof email === 'string' ? email : ''}</span>
              </p>
            </>
          )}
        </div>
        ...
        {error && (
          <div
            className='mb-6 p-4 rounded-2xl text-xs font-bold flex items-center gap-3 animate-scaleUp'
            style={{
        ...
        <div className='space-y-6'>
          {step === 'email' && (
            <form onSubmit={handleEmailStep} className='space-y-4'>
              <div className='group'>
                <label className='block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1 group-focus-within:text-primary transition-colors'>
                  E-mail Corporativo
                </label>
                <input
                  type='email'
              ...
              <button
                type='submit'
                disabled={loading || !isLoaded || !email.trim()}
          ...
          {step === 'password' && (
            <form onSubmit={handlePasswordStep} className='space-y-4'>
              ...
              <button
                type='submit'
                disabled={loading || !isLoaded || !password.trim()}
                className='w-full h-10 rounded-lg font-black text-[11px] text-white uppercase tracking-widest transition-all duration-300 auth-button-shimmer shadow-lg shadow-primary/5'
                style={{ background: 'linear-gradient(135deg, #13c8ec, #0891b2)' }}
              >
          ...
          {step === 'code' && (
            <form onSubmit={handleCodeStep} className='space-y-4'>
              <div className='group'>
                <label className='block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1 text-center'>
                  Código de 6 dígitos
                </label>
                <input
                  type='text'
        ...
        {step === 'email' && (
          <div className='mt-7 text-center'>
            <p className='text-[10px] font-bold text-slate-600 uppercase tracking-widest'>
              ...
              <Link
                to='/signup'
                className='text-primary hover:text-white transition-colors border-b border-primary/20 hover:border-white'
              >
                Cadastre-se →
              </Link>
            </p>

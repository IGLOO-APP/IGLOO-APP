                    ...
                    {u.role === 'owner' ? (
                      <div className='flex items-center gap-3'>
                        <div
                          ...
                          title={`${u.metrics?.properties || 0} Imóveis`}
                        >
                          <span className='text-[10px] font-black text-slate-900 dark:text-white'>
                            {u.metrics?.properties || 0}
                          </span>
                        ...
                        <div
                          className='flex flex-col items-center'
                          title={`${u.metrics?.tenants || 0} Inquilinos`}
                        >
                          <span className='text-[10px] font-black text-slate-900 dark:text-white'>
                            {u.metrics?.tenants || 0}
                          </span>
                        ...
                        <div
                          className='flex flex-col items-center'
                          title={`${u.metrics?.contracts || 0} Contratos`}
                        >
                          <span className='text-[10px] font-black text-slate-900 dark:text-white'>
                            {u.metrics?.contracts || 0}
                          </span>
                          <span className='text-[8px] font-bold text-slate-400 uppercase tracking-tighter'>
                            Cont
                          </span>
                      ...
                      <div className='relative'>
                        ...
                        {activeDropdown === `actions-${u.id}` && (
                          <>
                            ...
                            ></div>
                            <div
                              className={`absolute right-0 ${users.indexOf(u) >= users.length - 2 && users.length > 2 ? 'bottom-full mb-1' : 'top-full mt-1'} w-56 bg-white dark:bg-surface-dark rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 py-2 z-20 animate-scaleUp ${users.indexOf(u) >= users.length - 2 && users.length > 2 ? 'origin-bottom-right' : 'origin-top-right'}`}
                            >
                              {u.role === 'pending' && (
                                <button
                                  onClick={() => {
            ...
            <div className='flex items-center gap-1'>
              {Array.from({ length: Math.ceil(totalUsers / itemsPerPage) }, (_, i) => i + 1)
                .filter(
                  (p) =>
                    p === 1 ||
                    p === Math.ceil(totalUsers / itemsPerPage) ||

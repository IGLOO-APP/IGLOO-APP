...
const UserManagement: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const { users } = await adminService.getUsers(
        ...
        100,
        searchTerm,
        filterStatus === 'Suspenso' ? 'suspended' : undefined
      );
      setUsers(users);
    } catch (error) {
  ...
  return (
    <div className='p-8 space-y-6 animate-fadeIn'>
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl text-white font-bold shadow-xl animate-slideDown ${toast.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'}`}
        >
          {toast.message}
                    ...
                    <td className='px-8 py-5'>
                      <div className='flex items-center justify-end gap-2 group-hover:translate-x-0 translate-x-4 opacity-0 group-hover:opacity-100 transition-all duration-300'>
                        ...
                        <button
                          ...
                          className='p-2.5 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-500 hover:text-white transition-all text-xs font-bold uppercase tracking-tighter'
                        >
                          <Mail size={18} />
                        </button>
                        <button
                          onClick={() =>
                            u.is_suspended ? handleUnsuspend(u) : handleSuspendClick(u)
                          }

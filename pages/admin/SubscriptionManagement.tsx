    ...
    <div className='p-8 space-y-8 animate-fadeIn'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        {metrics.map((m) => (
          <div
            key={m.label}
            className='bg-white dark:bg-surface-dark p-6 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm'
          >
            ...
            {[
              ...
              <div key={p.name} className='space-y-2'>
                ...
                <div className='h-3 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden'>
                  <div
                    className={`h-full rounded-full bg-${p.color === 'primary' ? 'primary' : p.color + '-500'}`}

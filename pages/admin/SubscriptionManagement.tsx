...
const SubscriptionManagement: React.FC = () => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      tooltipDesc: 'Receita Recorrente Anual estimada com base no MRR atual.',
    },
    {
      label: 'Churn Rate',
      value: '0%',
      ...
      tooltipDesc: 'Percentual de proprietários que cancelaram no mês atual.',
    },
    {
      label: 'Ticket Médio',
      value: 'R$ 0,00',
            ...
            {(
              ...
              >
                ...
                <div className='flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-tighter'>
                  ...
                  <span>{p.isFree ? `${p.percentage}% da base` : `${p.percentage}% do total`}</span>
                </div>
                {p.isFree && (
          ...
          <div className='flex-1 h-64 min-h-[250px]'>
            <ResponsiveContainer width='100%' height='100%'>
              <BarChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <Bar dataKey='value' fill='#10b981' radius={[6, 6, 0, 0]} maxBarSize={40}>
                    ...
                    <div className='relative'>
                      ...
                      {newPlanMonthly && newPlanAnnual && (
                        <span className='absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded'>
                          -
                          {Math.round(
                            (1 - parseFloat(newPlanAnnual) / (parseFloat(newPlanMonthly) * 12)) *
                              100
                          )}
                          %
                        </span>
                ...
                <div>
                  ...
                  <div className='flex gap-2 mb-3'>
                    <input
                      ...
                      onKeyPress={(e) =>
                        e.key === 'Enter' &&
                        (setFeaturesList([...featuresList, newFeature]), setNewFeature(''))
                      }
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {featuresList.map((f, i) => (
                      <span
                        key={i}
                        className='flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 rounded-lg text-[10px] font-bold'
                      >
                        {f}
                        <button
                          onClick={() =>
                            setFeaturesList(featuresList.filter((_, idx) => idx !== i))
                          }
                        >

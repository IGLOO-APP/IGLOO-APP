    ...
    <div className='animate-fadeIn space-y-8 max-w-4xl'>
        ...
        <div className='space-y-4'>
          <div className='flex items-center justify-between p-4 rounded-xl border border-border bg-muted/35'>
            ...
            <button
              ...
              className={`transition-all ${config.accepts_deposit ? 'text-primary' : 'text-slate-300 dark:text-slate-600'}`}
            >
          ...
          <div className='flex items-center justify-between p-4 rounded-xl border border-border bg-muted/35'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <p className='font-bold text-sm text-slate-700 dark:text-slate-200'>Fiador</p>
                <p className='text-xs text-slate-400'>
            ...
            <button
              ...
              className={`transition-all ${config.accepts_guarantor ? 'text-primary' : 'text-slate-300 dark:text-slate-600'}`}
            >
          ...
          <div className='space-y-6'>
            <div className='flex items-center justify-between p-4 rounded-xl border border-border bg-muted/35'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  <p className='font-bold text-sm text-slate-700 dark:text-slate-200'>Pix</p>
                  <p className='text-xs text-slate-400'>Receber via chave Pix</p>
                </div>
              </div>
              <button
                onClick={() => update('pix_enabled', !config.pix_enabled)}
                className={`transition-all ${config.pix_enabled ? 'text-primary' : 'text-slate-300 dark:text-slate-600'}`}
              >
                {config.pix_enabled ? (
                  <ToggleRight size={32} strokeWidth={1.5} />
                ) : (
            ...
            <div className='flex items-center justify-between p-4 rounded-xl border border-border bg-muted/35'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  ...
                  <p className='text-xs text-slate-400'>Receber via transferência bancária</p>
                </div>
              </div>
              <button
                onClick={() => update('bank_transfer_enabled', !config.bank_transfer_enabled)}
                className={`transition-all ${config.bank_transfer_enabled ? 'text-primary' : 'text-slate-300 dark:text-slate-600'}`}
              >
      ...
      <div className='flex justify-end'>
        <button
          onClick={handleSave}
          disabled={saving}
          className='flex items-center gap-1.5 md:gap-2 px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl text-xs md:text-sm font-bold shadow-lg transition-all active:scale-95 shrink-0 bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-slate-900/20'
        >

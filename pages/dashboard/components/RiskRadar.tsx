...
export const RiskRadar: React.FC<RiskRadarProps> = ({ risks }) => {
  ...
  return (
    <div className='bg-[var(--card-bg)] p-6 rounded-[var(--card-radius)] border border-[var(--card-border)] shadow-sm'>
        ...
        {risks.length > 0 ? (
          risks.map((risk, idx) => (
            ...
            >
              ...
              <ChevronRight
                size={14}
                className='text-slate-300 group-hover:text-primary transition-all group-hover:translate-x-0.5'
              />
            </button>
          ))
        ) : (
          <div className='py-6 text-center'>
            ...
            <p className='text-xs font-bold text-emerald-500'>
              Carteira saudável — nenhum alerta no momento
            </p>
          </div>
        )}
      </div>

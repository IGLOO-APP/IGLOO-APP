...
export const DashboardAIInsights: React.FC<DashboardAIInsightsProps> = ({ metrics, onActionComplete }) => {
    ...
    <Card className='flex flex-col'>
      ...
      <CardContent className='space-y-3 flex flex-col flex-grow pt-4'>
        ...
        <div className='flex flex-col gap-2 p-4 rounded-[24px] bg-muted/50 border border-border/50'>
          ...
          {occupancyRate < 80 && (
            <Button
              onClick={() => setConfirmAction({ endpoint: '/api/imoveis/sugestao-preco', label: 'revisar os preços dos imóveis' })}
              variant='link'
              size='sm'
        ...
        {pendingMaintenanceCount > 0 && (
          <div className='flex flex-col gap-2 p-4 rounded-[24px] bg-muted/50 border border-border/50'>
            ...
            <Button
              onClick={() => setConfirmAction({ endpoint: '/api/manutencao/resolver', label: 'resolver os reparos pendentes' })}
              variant='link'
              size='sm'
              className='text-xs font-semibold self-end'
            >
